(function(){
  ...
  function renderMealSections(){
    ...
    MEAL_SLOTS.forEach(slot=>{
      ...
      el.innerHTML = items.slice().reverse().map(f=>{
  ...
  function renderEditorRows(errorMsg){
    ...
    html += editorRows.map(r=>{
      return '<div class="dishcard" data-row="'+r.id+'">'+
        ...
        '</div>'+
        '<div class="qtyrow">'+
          '<input type="text" class="qtyinput" data-field="portionEstimate" value="'+escapeHtml(r.portionEstimate)+'" placeholder="Quantity, e.g. 150g or 1 cup">'+
          '<button class="btn ghost qtybtn" data-recalc="'+r.id+'">↻ Recalc</button>'+
        '</div>'+
        '<div class="dishgrid">'+
    ...
    html += '<div id="addDishArea"><button class="btn secondary" id="addDishBtn">+ Add a dish it missed</button></div>'+
      '<div class="stack">'+
  ...
  function wireEditorEvents(){
    ...
    resultEl.querySelectorAll('[data-recalc]').forEach(btn=>{
      btn.addEventListener('click', ()=>recalcRow(btn.getAttribute('data-recalc'), btn));
    });
  ...
  // Re-asks the AI for just one dish's numbers after the user corrects its
  // quantity (e.g. changes "1 cup" to "150g") — keeps the dish name, sends
  // name + new quantity to /api/estimate, and overwrites that row's numbers.
  async function recalcRow(rowId, btn){
    const row = editorRows.find(r=>r.id===rowId);
    if(!row) return;
    const card = btn.closest('.dishcard');
    const nameInput = card.querySelector('[data-field="name"]');
    const qtyInput = card.querySelector('[data-field="portionEstimate"]');
    row.name = nameInput.value;
    row.portionEstimate = qtyInput.value;
    if(!row.name.trim()){ toast('Give the dish a name first'); return; }
    ...
    try{
      const res = await fetch('/api/estimate', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ name: row.name, quantity: row.portionEstimate })
      });
      if(!res.ok){
        const errBody = await res.json().catch(()=>({}));
        throw new Error(errBody.error || ('Server returned '+res.status));
      ...
      const data = await res.json();
      row.calories = data.calories||0;
      row.protein_g = data.protein_g||0;
      row.carbs_g = data.carbs_g||0;
      row.fat_g = data.fat_g||0;
      row.fiber_g = data.fiber_g||0;
      row.confidence = data.confidence||null;
      if(data.portionEstimate) row.portionEstimate = data.portionEstimate;
      renderEditorRows();
      toast('Recalculated');
    }catch(err){
  ...
  // Inline "add a missed dish" form: type a description like "1 large
  // boiled egg" and either get an AI estimate or add it as a blank row
  // to fill in by hand.
  function showAddDishForm(){
    const area = document.getElementById('addDishArea');
    area.innerHTML =
      '<div class="dishcard">'+
        '<input type="text" id="newDishDesc" placeholder="e.g. 1 large boiled egg, or Butter chicken 250g">'+
        '<div class="stack" style="margin-top:10px;">'+
          '<button class="btn" id="estimateDishBtn">Estimate with AI</button>'+
          '<button class="btn ghost" id="blankDishBtn">Add without AI (fill in manually)</button>'+
        '</div>'+
      '</div>';
    document.getElementById('blankDishBtn').addEventListener('click', ()=>{
      editorRows.push(blankRow());
      renderEditorRows();
    });
    document.getElementById('estimateDishBtn').addEventListener('click', async (e)=>{
      const desc = document.getElementById('newDishDesc').value.trim();
      if(!desc){ toast('Type what the dish is first'); return; }
      const btn = e.target;
      const originalText = btn.textContent;
      btn.textContent = '…'; btn.disabled = true;
      try{
        ...
        toast('Couldn\'t estimate that (' + (err.message||'error') + ') — add it manually instead');
        btn.textContent = originalText; btn.disabled = false;
      }
    });
  }
